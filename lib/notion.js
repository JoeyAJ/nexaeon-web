...
export function getNotionClient(apiKey) {
  ...
  if (!notionClients.has(apiKey)) {
    notionClients.set(apiKey, new Client({ auth: apiKey, timeoutMs: NOTION_TIMEOUT_MS }));
  }
...
export function getNotionIdentityConfig() {
  const apiKey = getEnvValue('NOTION_API_KEY');
  const databaseId = getEnvValue('NOTION_IDENTITY_DATABASE_ID');
...
export async function queryAllNotionDatabasePages(databaseId, apiKey, queryOptions = {}) {
  const notion = getNotionClient(apiKey);
  if (!notion) throw new Error('Notion client is not configured');
  ...
  // Environment variables named *_DATABASE_ID contain a Notion database ID.
  // Resolve its current data source before querying so the SDK does not emit an
  // expected object_not_found warning for every successful cold-start request.
  const database = await notion.databases.retrieve({ database_id: databaseId });
  const dataSourceId = database?.data_sources?.[0]?.id;
  if (!dataSourceId) {
    throw new Error('Notion database does not expose a queryable data source');
  }

  async function queryPage(targetDataSourceId, startCursor) {
    return notion.dataSources.query({
  ...
  return collectPaginatedNotionResults(async (startCursor) => {
    return queryPage(dataSourceId, startCursor);
  });
}

export async function collectPaginatedNotionResults(queryPage, { maxPages = NOTION_MAX_PAGES } = {}) {
  const results = [];
  ...
  while (pageCount < maxPages) {
    const payload = await queryPage(startCursor);
