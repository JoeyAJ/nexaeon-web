...
export const XCHANGE_MODEL_MODES = Object.freeze(['rules', 'shadow', 'live', 'disabled']);
export const MODEL_GATEWAY_LIMITS = Object.freeze({
...
export function getModelConfiguration(env = process.env) {
  ...
  const requestedFallbackProvider = String(env.NEXAEON_MODEL_FALLBACK || 'mock').trim().toLowerCase();
  return Object.freeze({
    provider,
    requestedProvider,
    model: String(env.NEXAEON_MODEL_NAME || env.OPENAI_MODEL || DEFAULT_OPENAI_MODEL).trim(),
    apiKey: String(env.OPENAI_API_KEY || '').trim(),
    fallbackProvider: requestedFallbackProvider === 'mock' ? 'mock' : 'disabled',
...
export function getXchangeModelMode(env = process.env) {
  const mode = String(env.NEXAEON_XCHANGE_MODEL_MODE || 'rules').trim().toLowerCase();
  if (!XCHANGE_MODEL_MODES.includes(mode)) {
    throw Object.assign(new Error('model_mode_invalid'), { code: 'MODEL_MODE_INVALID', status: 503 });
  }
  return mode;
}
