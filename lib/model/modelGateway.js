...
function metadata({ provider, model, fallbackUsed, requestId, startedAt, usage, fallbackReason = null }) {
  return Object.freeze({
    provider, model, generationMode: fallbackUsed ? 'fallback' : provider === 'mock' ? 'deterministic' : 'real',
...
export function createModelGateway({ env = process.env, registry, openaiClient } = {}) {
  ...
  async function invoke(kind, request) {
    ...
    return {
      ...
      metadata: metadata({ provider: selected, model: result.model, fallbackUsed: Boolean(fallbackReason), requestId, startedAt, usage: result.usage, fallbackReason }),
    };
  }
