...
// Optional Networker fields must never fall back to an unrelated property of
// the same Notion type. Scope the generic readers to an exact-name match only.
function getOptionalPropertyScope(properties, names) {
  const property = findProperty(properties, names);
  return property ? { [names[0]]: property } : {};
}

function getOptionalList(properties, names) {
  return getMultiSelect(getOptionalPropertyScope(properties, names), names);
}

function getOptionalText(properties, names) {
  return getText(getOptionalPropertyScope(properties, names), names);
}

function getOptionalUrl(properties, names) {
  return getUrl(getOptionalPropertyScope(properties, names), names);
}
...
export function normalizeIdentityProfile(page) {
  ...
  return {
    ...
    expertise: getOptionalList(properties, FIELD_NAMES.expertise),
    researchInterests: getOptionalList(properties, FIELD_NAMES.researchInterests),
    organizations: getOptionalList(properties, FIELD_NAMES.organizations),
    collaborationInterests: getOptionalList(properties, FIELD_NAMES.collaborationInterests),
    projects: getOptionalList(properties, FIELD_NAMES.projects),
    languages: getOptionalList(properties, FIELD_NAMES.languages),
    region: getOptionalText(properties, FIELD_NAMES.region),
    publicContact: getOptionalList(properties, FIELD_NAMES.publicContact),
    profileUrl: getOptionalUrl(properties, FIELD_NAMES.profileUrl),
