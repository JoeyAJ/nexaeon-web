...
import { createMemoryAuditRepository } from './auditRepository.js';
...
function auditBase({ operationId, actor, tool, safePayload, previewHash, idempotencyKey, status, confirmation = false, now = Date.now() }) {
  return {
    operationId, timestamp: new Date(now).toISOString(), actorId: actor.actorId, actorRole: actor.role,
    actorSessionId: actor.sessionId, agentId: 'orchestrator', toolId: ACTION_DRAFT_TOOL_ID,
...
async function persistAudit(repository, method, args, fallbackEvent, logger) {
  try {
    ...
    return { auditRecordId: null, auditPersistenceStatus: 'fallback_log' };
  }
}
...
export async function createOperationPreview({ payload, req, actor, env = process.env, now = Date.now(), operationId = randomUUID(), auditRepository = defaultAuditRepository, logger = console.error }) {
  ...
  const confirmationToken = createConfirmationToken({ operationId, agentId, toolId, targetDataSource, previewHash, idempotencyKey, fingerprint, expiresAt, actorId: admin.actorId, actorRole: admin.role, actorSessionId: admin.sessionId }, env);
  const audit = auditBase({ operationId, actor: admin, tool, safePayload, previewHash, idempotencyKey, status: OPERATION_STATUS.PREVIEWED, now });
  ...
  const preview = {
    ...
    fieldsToWrite: getActionDraftFields(safePayload, idempotencyKey),
    warnings: ['Creates one hidden Action Center draft. Status and Visibility remain unset because no compatible Draft select option is verified in the production schema.', 'The [Draft] primary-field prefix is the safe state and idempotency boundary.', 'Does not activate work, notify anyone, or execute follow-up actions.', 'Rollback is not supported.'],
  ...
  operationStore.set(operationId, { status: OPERATION_STATUS.PREVIEWED, expiresAt, idempotencyKey, actorId: admin.actorId, actorSessionId: admin.sessionId, previewHash, safePayload });
  return preview;
}
...
export function logToolAudit(event, logger = console.error) {
  const safeEvent = {
    ...
    actorId: cleanText(event.actorId, 160), actorRole: cleanText(event.actorRole, 40), actorSessionId: cleanText(event.actorSessionId, 80),
...
export async function executeConfirmedOperation({ body, req, actor, env = process.env, now = Date.now(), createDraft = createAirtableActionDraft, auditRepository = defaultAuditRepository, logger = console.error }) {
  ...
  try {
    ...
    verifyConfirmationToken(body?.confirmationToken, { env, now, fingerprint: createRequesterFingerprint(req), expected: { operationId, agentId, toolId, targetDataSource, previewHash, idempotencyKey, actorId: admin.actorId, actorRole: admin.role, actorSessionId: admin.sessionId } });
    ...
    const audit = auditBase({ operationId, actor: admin, tool, safePayload, previewHash, idempotencyKey, status: OPERATION_STATUS.EXECUTING, confirmation: true, now });
    if (stored?.status === OPERATION_STATUS.SUCCEEDED && stored.idempotencyKey === idempotencyKey) {
      ...
      return { ok: true, operationId, executionStatus: OPERATION_STATUS.SUCCEEDED, targetDataSource, externalRecordId: stored.externalRecordId, idempotencyKey, replayed: true, ...persistence };
    }
    ...
    operationStore.set(operationId, { status: OPERATION_STATUS.SUCCEEDED, ...baseOutput, actorId: admin.actorId, actorSessionId: admin.sessionId });
    const finalAudit = { ...audit, executionStatus: OPERATION_STATUS.SUCCEEDED, externalRecordId: result.externalRecordId, replayed: Boolean(result.replayed), duration: Date.now() - startedAt };
    const persistence = await persistAudit(auditRepository, 'updateAuditExecutionResult', [operationId, finalAudit], finalAudit, logger);
    logToolAudit(finalAudit, logger);
    return { ...baseOutput, ...persistence };
  } catch (error) {
    ...
    const failedAudit = { operationId, timestamp: new Date(now).toISOString(), actorId: admin.actorId, actorRole: admin.role, actorSessionId: admin.sessionId, agentId, toolId, permissionLevel: tool?.permissionLevel || '', targetDataSource, actionType: 'create', userConfirmation: Boolean(body?.confirmationToken), confirmationTimestamp: body?.confirmationToken ? new Date(now).toISOString() : null, sanitizedInput: safePayload, previewHash, idempotencyKey, executionStatus: status, errorCode: error.code || 'EXECUTION_FAILED', duration: Date.now() - startedAt, source: 'orchestrator-action-draft' };
...
export async function cancelOperation({ body, actor, auditRepository = defaultAuditRepository, logger = console.error }) {
  const admin = requireAdminActor(actor);
  const operationId = cleanText(body?.operationId, 80);
  ...
  const audit = { operationId, actorId: admin.actorId, actorRole: admin.role, actorSessionId: admin.sessionId, agentId: 'orchestrator', toolId: ACTION_DRAFT_TOOL_ID, permissionLevel: 'WRITE_CONFIRM', targetDataSource: ACTION_DRAFT_DATA_SOURCE, actionType: 'create', userConfirmation: false, sanitizedInput: current?.safePayload || {}, previewHash: current?.previewHash, executionStatus: OPERATION_STATUS.CANCELLED, idempotencyKey: current?.idempotencyKey, source: 'orchestrator-action-draft' };
  const persistence = await persistAudit(auditRepository, 'updateAuditExecutionResult', [operationId, audit], audit, logger);
  logToolAudit(audit, logger);
