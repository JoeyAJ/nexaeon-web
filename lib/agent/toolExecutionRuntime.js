...
export async function executeConfirmedOperation({ body, req, actor, env = process.env, now = Date.now(), createDraft = createAirtableActionDraft, linkDraft = linkAirtableActionDraftAudit, auditRepository = defaultAuditRepository, logger = console.error }) {
