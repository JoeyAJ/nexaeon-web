...
function sanitize(value, depth = 0, path = []) {
  if (depth > 6) return '[truncated]';
  if (Array.isArray(value)) return value.slice(0, 100).map((item, index) => sanitize(item, depth + 1, [...path, String(index)]));
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value)
      ...
      .map(([key, item]) => {
        ...
        return [safeKey, sanitize(item, depth + 1, [...path, safeKey])];
      }));
  }
...
async function request(url, options, fetchImpl, context = {}) {
  ...
  try { payload = await response.json(); } catch {
    if (response.ok) throw Object.assign(new Error('audit_invalid_response'), { code: 'AUDIT_INVALID_RESPONSE', status: response.status, operation: context.operation, diagnosticReason: 'invalid_json' });
    payload = {};
  }
  if (!response.ok) {
    const upstreamError = payload?.error;
    const airtableErrorType = clean(typeof upstreamError === 'string' ? upstreamError : upstreamError?.type, 100);
    const upstreamMessage = clean(typeof upstreamError === 'object' ? upstreamError?.message : '', 300);
    let diagnosticReason = 'request_rejected';
    if (response.status === 401) diagnosticReason = 'authentication_failed';
    ...
    else if (response.status === 422 && /unknown field|field.*not found/iu.test(`${airtableErrorType} ${upstreamMessage}`)) diagnosticReason = 'field_missing';
    ...
    else if (response.status === 422) diagnosticReason = 'field_or_schema_invalid';
...
function toAirtableFields(record) {
  ...
  return {
    ...
    'Sanitized Output': safeJson(sanitizedOutput),
...
export function createAirtableAuditRepository({ env = process.env, fetchImpl = fetch, timeoutMs = 8_000 } = {}) {
  ...
  async function append(record) {
    ...
    }, fetchImpl, { tableRole: 'audit', operation: 'append_audit_record', fieldNames: Object.keys(fields) });
  ...
  const repository = {
    ...
    async acquireExecutionLock(record) {
      ...
      try {
        ...
        }, fetchImpl, { tableRole: 'audit', operation: 'acquire_execution_lock', fieldNames: ['Audit ID', ...Object.keys(toAirtableFields(normalized)).filter((name) => name !== 'Audit ID')] });
      } catch (error) {
