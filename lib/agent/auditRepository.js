...
export const AUDIT_RECORD_TYPE = Object.freeze({ FORMAL: 'formal', LEGACY: 'legacy', MIGRATED: 'legacy-migrated' });
...
export function createAirtableAuditRepository({ env = process.env, fetchImpl = fetch, timeoutMs = 8_000 } = {}) {
  ...
  async function listDedicated() {
    ...
    do {
      ...
      records.push(...(page.records || []).map(fromAirtableRecord));
      offset = page.offset || '';
  ...
  async function listLegacy() {
    ...
    query.searchParams.set('pageSize', '100');
    query.searchParams.set('filterByFormula', `LEFT({Project Name}, ${LEGACY_RECORD_PREFIX.length})=${JSON.stringify(LEGACY_RECORD_PREFIX)}`);
    const page = await request(query, { headers, signal: AbortSignal.timeout(timeoutMs) }, fetchImpl);
    return (page.records || []).map(fromLegacyRecord);
  }
  ...
  const repository = {
    ...
    async listAuditRecordsForAdmin(filters = {}) {
      const formal = filters.recordType === 'legacy' ? [] : await listDedicated();
      const legacy = filters.recordType === 'formal' ? [] : await listLegacy();
