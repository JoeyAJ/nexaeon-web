...
export function canonicalizeXchangeProperties(properties = {}) {
  const canonical = {};
  for (const name of PROPERTY_NAMES) if (properties[name]) canonical[name] = propertyTypeValue(properties[name]);
  ...
  if (publishedEntry) canonical.published = propertyTypeValue(publishedEntry[1]);
  return canonical;
}
...
export function canonicalizeXchangeBlock(block = {}) {
  ...
  if (Array.isArray(block.children) && block.children.length) canonical.children = block.children.map(canonicalizeXchangeBlock);
  return canonical;
}

export function canonicalizeXchangeBlocks(blocks = []) { return blocks.map(canonicalizeXchangeBlock); }
...
function resolveTarget(lifecycle) {
  ...
  const required = snapshot && snapshot.expectedProperties && snapshot.contentPreview && snapshot.contentSchemaVersion && snapshot.rendererVersion
    && Number.isInteger(snapshot.estimatedBodyBlocks) && snapshot.durationValidation && snapshot.executedPreviewHash && snapshot.parentDataSourceId;
...
function validationAudit({ validationOperationId, executeOperationId, target, snapshot, actor, req, now, event, output = {}, errorCode = null }) {
  ...
  return {
    ...
    actorSessionHash: hashActorSession(actor.sessionId), agentId: 'xchange', toolId: target?.toolId || '', permissionLevel: 'READ_VALIDATE',
    targetDataSource: TARGET, actionType: 'validate', executionStatus: event === 'validation_failed' ? 'failed' : event === 'validation_succeeded' ? 'succeeded' : 'executing',
    confirmationStatus: 'not_required', sanitizedInput: { executeOperationId }, sanitizedOutput: {
      validationEvent: event, executeOperationId, parentOperationId: snapshot?.parentOperationId || null,
...
async function persistValidationAudit(repository, event) {
  if (!repository?.updateAuditExecutionResult) fail('AUDIT_CONFIGURATION_MISSING');
  try { await repository.updateAuditExecutionResult(event.operationId, event); }
  catch { throw Object.assign(new Error('audit_persistence_failed'), { code: 'AUDIT_PERSISTENCE_FAILED', writesPerformed: 0 }); }
}

export async function validateXchangeDraftDelivery({ body, req, actor, auditRepository, notionReader = readXchangeNotionDraft, notionClient, env = process.env, now = Date.now(), validationOperationId = randomUUID() } = {}) {
  ...
  catch { fail('AUDIT_PERSISTENCE_FAILED'); }
