...
function airtableFailure(response, payload, context = {}) {
  const upstreamType = clean(payload?.error?.type, 100) || 'UNKNOWN_AIRTABLE_ERROR';
  const message = clean(payload?.error?.message, 1000);
  const knownFieldNames = Array.isArray(context.fieldNames) ? context.fieldNames : [];
  const fieldName = knownFieldNames.find((name) => message.includes(name)) || '';
  const upper = `${upstreamType} ${message}`.toUpperCase();
  let code = 'DATA_SOURCE_REJECTED';
  if (response.status === 422) {
    if (/UNKNOWN_FIELD|FIELD_NOT_FOUND|CANNOT_FIND_FIELD/u.test(upper)) code = 'DATA_SOURCE_FIELD_MISSING';
    else if (/LINK|RECORD_ID/u.test(upper)) code = 'DATA_SOURCE_LINK_INVALID';
    else if (/TYPE|COLUMN|MULTIPLE_CHOICE|INVALID_VALUE/u.test(upper)) code = 'DATA_SOURCE_FIELD_TYPE_INVALID';
    else code = 'DATA_SOURCE_SCHEMA_INVALID';
  }
  return Object.assign(new Error('migration_request_rejected'), {
    code, status: response.status, airtableErrorType: upstreamType,
    tableRole: clean(context.tableRole, 40), tableName: clean(context.tableName, 120), fieldName,
  });
}
...
async function request(url, options, fetchImpl, context = {}) {
  ...
  if (!response.ok) {
    let payload = null;
    try { payload = await response.json(); } catch { /* upstream body is intentionally not surfaced */ }
    throw airtableFailure(response, payload, context);
  }
  try {
...
export function createAirtableMigrationDataSource({ env = process.env, fetchImpl = fetch, timeoutMs = 8_000 } = {}) {
  ...
  async function listTable(tableId) {
    ...
    do {
      ...
      const role = tableId === cfg.projectsTableId ? 'projects' : 'audit';
      const page = await request(url, { headers, signal: AbortSignal.timeout(timeoutMs) }, fetchImpl, { tableRole: role });
  ...
  return {
    describeTarget() {
      return { baseId: cfg.baseId, projectsTableId: cfg.projectsTableId, auditTableId: cfg.auditTableId };
    },
    async inspectSchema() {
      let metadata;
      try {
        metadata = await request(`${AIRTABLE_API_ROOT}/meta/bases/${encodeURIComponent(cfg.baseId)}/tables`, { headers, signal: AbortSignal.timeout(timeoutMs) }, fetchImpl, { tableRole: 'metadata' });
      } catch (error) {
        if (error?.status === 403 && error?.airtableErrorType === 'INVALID_PERMISSIONS_OR_MODEL_NOT_FOUND') {
          throw Object.assign(new Error('migration_schema_metadata_forbidden'), { ...error, code: 'DATA_SOURCE_SCHEMA_METADATA_FORBIDDEN' });
        }
        throw error;
      }
      const selected = (metadata.tables || []).filter(({ id }) => id === cfg.projectsTableId || id === cfg.auditTableId);
      return selected.map((table) => ({
        role: table.id === cfg.projectsTableId ? 'projects' : 'audit', tableId: clean(table.id, 120), name: clean(table.name, 120), primaryFieldId: clean(table.primaryFieldId, 120),
        fields: (table.fields || []).map((field) => ({
          id: clean(field.id, 120), name: clean(field.name, 160), type: clean(field.type, 80), isPrimary: field.id === table.primaryFieldId,
          linkedTableId: clean(field.options?.linkedTableId, 120),
          choices: Array.isArray(field.options?.choices) ? field.options.choices.map((choice) => clean(choice?.name, 160)).filter(Boolean) : [],
        })),
      }));
    },
    ...
    async createAudit(fields) {
      ...
      }, fetchImpl, { tableRole: 'audit', fieldNames: Object.keys(fields) });
    ...
    async updateProject(recordId, fields) {
      ...
      }, fetchImpl, { tableRole: 'projects', fieldNames: Object.keys(fields) });
