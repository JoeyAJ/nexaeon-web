...
export function getNetworkerProductionConfig(env = process.env) {
  const navigatorConfig = getNavigatorProductionConfig(env);
  const networkerEnabled = hasOwn(env, 'NEXAEON_NETWORKER_ENABLED')
    ? parseBoolean(env.NEXAEON_NETWORKER_ENABLED)
    : true;

  return {
    ...navigatorConfig,
    enabled: navigatorConfig.enabled && networkerEnabled,
    forceSourcesOnly: hasOwn(env, 'NEXAEON_NETWORKER_FORCE_SOURCES_ONLY')
      ? parseBoolean(env.NEXAEON_NETWORKER_FORCE_SOURCES_ONLY)
      : navigatorConfig.forceSourcesOnly,
  };
}
