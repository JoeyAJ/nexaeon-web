...
function conceptDiagnostic(value = {}) {
  return Object.freeze({
    matchedConcepts: boundedArray(value.matchedConcepts, MAX_CODES),
    missingConcepts: boundedArray(value.missingConcepts, MAX_CODES),
  });
}
...
export function buildShadowQualityDiagnostic(quality) {
  ...
  return Object.freeze({
    ...
    locale: safeReason(quality.locale || '').slice(0, 8) || null,
    semanticDiagnostics: Object.freeze({
      topic: conceptDiagnostic(quality.semanticDiagnostics?.topic),
      objectives: conceptDiagnostic(quality.semanticDiagnostics?.aiMarketing?.objectives),
      assessment: conceptDiagnostic(quality.semanticDiagnostics?.aiMarketing?.assessment),
      risks: conceptDiagnostic(quality.semanticDiagnostics?.aiMarketing?.risks),
    }),
    objectiveVerbMatches: Object.freeze((quality.objectiveVerbMatches || []).slice(0, 6).map((match) => Object.freeze({
      objectiveIndex: numericDiagnostic(match.objectiveIndex),
      matchedVerb: safeReason(match.matchedVerb || '').slice(0, 40) || null,
      valid: match.valid === true,
    }))),
    thresholds: Object.freeze({
      topicRelevance: numericDiagnostic(quality.thresholds?.topicRelevance),
      promptOverlap: numericDiagnostic(quality.thresholds?.promptOverlap),
      aiMarketingObjectives: numericDiagnostic(quality.thresholds?.aiMarketingObjectives),
      groupActivityConcepts: numericDiagnostic(quality.thresholds?.groupActivityConcepts),
      assessmentConcepts: numericDiagnostic(quality.thresholds?.assessmentConcepts),
      riskConcepts: numericDiagnostic(quality.thresholds?.riskConcepts),
    }),
    validatorVersion: safeReason(quality.validatorVersion || '').slice(0, 40) || null,
