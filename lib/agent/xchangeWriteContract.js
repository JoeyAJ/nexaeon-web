...
async function findReusablePreview({ idempotencyKey, actorSessionHash, now, auditRepository }) {
  ...
  return {
    ...
    modelGeneration: output.modelGeneration || {
      mode: 'rules', provider: 'mock', requestedProvider: 'mock', actualProvider: 'mock',
      model: 'deterministic-v1', generationMode: 'rules', fallbackUsed: false,
    ...
    shadowComparison: output.shadowComparison || null,
    rejectedFields: [],
    warnings: [
...
function rulesMetadata({ config, requestId, generatedAt = new Date().toISOString(), mode = 'rules' }) {
  return {
    ...
    generationMode: mode, fallbackUsed: false, requestId, generatedAt, latencyMs: 0, tokenUsage: null,
  };
}

function shadowMetrics({ generation, quality, errorCode = null }) {
  ...
  if (body.draftType === XCHANGE_DRAFT_TYPES.COURSE && mode === 'shadow') {
    ...
    try {
      const generation = await (modelGateway || createModelGateway({ env })).structuredGenerate({
        ...prompt, requestId, task: 'xchange.course_draft.shadow', schemaName: XCHANGE_COURSE_SCHEMA_NAME,
      ...
      shadowComparison = shadowMetrics({ errorCode: error?.code || 'MODEL_PROVIDER_ERROR' });
    }
