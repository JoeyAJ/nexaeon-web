...
export async function executeXchangeDraft({ body, req, actor, auditRepository, notionWriter = createXchangeNotionDraft, now = Date.now(), env = process.env, logger = console.error }) {
  ...
  try {
    ...
    const successAudit = executionAudit(preview, actor, req, now, {
      ...
      sanitizedOutput: { requestId: preview.requestId, startedAt: confirmedAt, completedAt, estimatedWrites: 1, writesPerformed: 1, createdAt: completedAt, notionPageCreated: true, externalRecordId: result.externalRecordId, bodyComplete: result.bodyComplete !== false, bodyBlocksWritten: result.bodyBlocksWritten || 0, bodyAppendBatches: result.bodyAppendBatches || 0, partialExternalWrite: false },
    });
