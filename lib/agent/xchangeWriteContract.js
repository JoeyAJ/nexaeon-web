...
async function persistLifecycle(auditRepository, operationId, event) {
...
async function persistMissingPreviewFailure(auditRepository, operationId, actor, req, now, errorCode) {
...
export async function executeXchangeDraft({ body, req, actor, auditRepository, notionWriter = createXchangeNotionDraft, now = Date.now(), env = process.env }) {
  if (!actor?.actorId || actor.role !== 'admin' || !actor.sessionId) fail('AUTH_REQUIRED');
  ...
  if (!operationId) {
    await persistMissingPreviewFailure(auditRepository, '', actor, req, now, 'OPERATION_NOT_FOUND');
  ...
  if (!preview) {
    await persistMissingPreviewFailure(auditRepository, operationId, actor, req, now, 'PREVIEW_NOT_FOUND');
  ...
  if (!claim.acquired) {
    const current = await auditRepository.getAuditLifecycleByOperationId(operationId);
  ...
  try {
    ...
    await persistLifecycle(auditRepository, operationId, successAudit);
    ...
    await persistLifecycle(auditRepository, operationId, failed);
    throw error;
  }
}
