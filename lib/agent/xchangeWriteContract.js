...
export async function createXchangeDraftPreview({ body, req, actor, auditRepository, now = Date.now(), operationId = randomUUID(), requestId = randomUUID(), env = process.env }) {
  ...
  const { content: contentPreview, quality: contentQuality } = createStructuredContent(body.draftType, normalizedPayload, { requirements: extractedRequirements });
  if (!['Complete', 'Complete with warnings'].includes(contentQuality.status)) fail('CONTENT_VALIDATION_FAILED');
...
export async function reviseXchangeDraftPreview({ body, req, actor, auditRepository, now = Date.now(), operationId = randomUUID(), requestId = randomUUID(), env = process.env }) {
  ...
  if (lifecycle.some((record) => record.sanitizedOutput?.auditEvent === 'preview_superseded')) fail('CONFIRMATION_MISMATCH');
...
export async function executeXchangeDraft({ body, req, actor, auditRepository, notionWriter = createXchangeNotionDraft, now = Date.now(), env = process.env, logger = console.error }) {
  ...
  if (lifecycle.some((record) => record.sanitizedOutput?.auditEvent === 'preview_superseded' || record.source === 'xchange-preview-superseded')) fail('CONFIRMATION_MISMATCH');
  ...
  try {
    ...
    const successAudit = executionAudit(preview, actor, req, now, {
      ...
      sanitizedOutput: { requestId: preview.requestId, startedAt: confirmedAt, completedAt, estimatedWrites: 1, writesPerformed: 1, createdAt: completedAt, notionPageCreated: true, bodyComplete: result.bodyComplete !== false, bodyBlocksWritten: result.bodyBlocksWritten || 0, bodyAppendBatches: result.bodyAppendBatches || 0, partialExternalWrite: false },
    });
    ...
    const failed = executionAudit(preview, actor, req, now, {
      ...
      sanitizedOutput: { requestId: preview.requestId, startedAt: confirmedAt, completedAt: new Date().toISOString(), estimatedWrites: 1, writesPerformed: error?.writesPerformed || 0, notionPageCreated: Boolean(error?.notionPageCreated), pageCreated: Boolean(error?.pageCreated), bodyComplete: error?.bodyComplete !== false && !error?.partialExternalWrite, bodyBlocksWritten: error?.bodyBlocksWritten || 0, bodyAppendBatches: error?.bodyAppendBatches || 0, partialExternalWrite: Boolean(error?.partialExternalWrite) },
    });
