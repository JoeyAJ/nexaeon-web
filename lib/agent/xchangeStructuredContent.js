...
export const XCHANGE_MEASURABLE_OBJECTIVE_VERBS = Object.freeze({
  zh: Object.freeze(['辨識', '說明', '解釋', '比較', '應用', '建立', '評估', '設計', '分析']),
  ko: Object.freeze(['정의', '설명', '비교', '적용', '작성', '평가', '설계', '분석']),
  en: Object.freeze(['Identify', 'Explain', 'Compare', 'Apply', 'Create', 'Evaluate', 'Design', 'Analyze', 'Define', 'Demonstrate']),
});

const ENGLISH_MEASURABLE_VERBS = XCHANGE_MEASURABLE_OBJECTIVE_VERBS.en.join('|');
const CJK_MEASURABLE_VERBS = [...XCHANGE_MEASURABLE_OBJECTIVE_VERBS.zh, ...XCHANGE_MEASURABLE_OBJECTIVE_VERBS.ko].join('|');
const MEASURABLE_VERBS = new RegExp(`(?:${ENGLISH_MEASURABLE_VERBS}|${CJK_MEASURABLE_VERBS})`, 'iu');
const TRADITIONAL_CHINESE_VERB_AT_START = new RegExp(`^(?:${XCHANGE_MEASURABLE_OBJECTIVE_VERBS.zh.join('|')})`, 'u');
const HAS_HAN_SCRIPT = /\p{Script=Han}/u;
...
export function validateStructuredContent(draftType, content, { allowedUrls = [], requirements, sourcePrompt = '' } = {}) {
  ...
  if (course && content?.learningObjectives?.some((item) => {
    const objective = String(item || '').trim();
    return HAS_HAN_SCRIPT.test(objective) ? !TRADITIONAL_CHINESE_VERB_AT_START.test(objective) : !MEASURABLE_VERBS.test(objective);
  })) errors.push('Learning objectives must use measurable verbs.');
