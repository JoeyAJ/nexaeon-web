...
const MEASURABLE_VERBS = /(?:identify|explain|compare|apply|create|evaluate|design|analyze|define|demonstrate|辨識|說明|比較|應用|建立|評估|設計|分析|정의|설명|비교|적용|작성|평가|설계|분석)/iu;
...
export function generateCourseContent(payload) {
  const language = languageOf(payload); const t = TEXT[language]; const m = META[language];
  const topic = payload.subTopic || payload.title; const detail = payload.summary || '';
  const durations = splitDuration(payload.durationMinutes, [0.2, 0.35, 0.45]);
  return {
    ...
    coreContent: t.coreTitles.map((title, index) => ({ title, explanation: t.explanations(topic, detail)[index], keyPoints: t.keyPoints[index] })),
...
export function generateLearningActivityContent(payload) {
  const language = languageOf(payload); const t = TEXT[language]; const m = META[language]; const topic = payload.instructions || payload.activityTitle;
...
export function validateStructuredContent(draftType, content, { allowedUrls = [] } = {}) {
  ...
  const status = errors.length ? (errors.some((item) => /URL|maximum/iu.test(item)) ? 'Rejected' : 'Incomplete') : warnings.length ? 'Complete with warnings' : 'Complete';
  return { status, errors, warnings, durationValidation: { expectedMinutes: expectedDuration, actualMinutes: durationTotal, valid: expectedDuration === durationTotal }, estimatedBodyBlocks };
}
...
export function createStructuredContent(draftType, payload) {
  const content = draftType === 'course' ? generateCourseContent(payload) : generateLearningActivityContent(payload);
  const allowedUrls = [payload.fileUrl, payload.materialsUrl].filter(Boolean);
  return { content, quality: validateStructuredContent(draftType, content, { allowedUrls }) };
}
