import {
  AI_MARKETING_ACTIVITY_CONCEPTS,
  AI_MARKETING_ASSESSMENT_CONCEPTS,
  AI_MARKETING_TOPIC_CONCEPTS,
  AI_RISK_CONCEPTS,
  XCHANGE_MEASURABLE_OBJECTIVE_VERBS,
  getSemanticTerms,
  matchAnySemanticConcept,
  matchGenericTemplateIndicators,
  matchMeasurableObjectiveVerb,
  matchSemanticConcept,
  normalizeLocale,
} from './xchangeSemanticRegistry.js';
...
export { XCHANGE_MEASURABLE_OBJECTIVE_VERBS };
...
function subjectSemantics(title, prompt, locale) {
  const combined = `${title} ${prompt}`;
  const aiMarketing = matchSemanticConcept(combined, 'GENERATIVE_AI', locale).matched
    && (matchSemanticConcept(combined, 'MARKETING_STRATEGY', locale).matched || matchSemanticConcept(combined, 'AI_MARKETING', locale).matched);
  if (aiMarketing) return {
    subjectConcepts: [...AI_MARKETING_TOPIC_CONCEPTS],
    subjectKeywords: AI_MARKETING_TOPIC_CONCEPTS.map((conceptId) => getSemanticTerms(conceptId, locale)[0]),
  };
  return {
    subjectConcepts: [],
    subjectKeywords: [...new Set(String(title || '').split(/[\s、，,／/與和:&]+/u).map((item) => item.trim()).filter((item) => item.length >= 2))].slice(0, 12),
  };
}
...
export function extractStructuredRequirements(draftType, payload) {
  ...
  const semantics = subjectSemantics(exactTitle, prompt, language);
  ...
  return Object.freeze({
    ...
    requiredElements, subjectConcepts: semantics.subjectConcepts, subjectKeywords: semantics.subjectKeywords,
  });
}
...
function isAiMarketing(requirements) {
  return requirements?.subjectConcepts?.includes('GENERATIVE_AI') && requirements.subjectConcepts.includes('MARKETING_STRATEGY');
}
...
function generalCourseContent(payload, requirements) {
...
const AI_MARKETING_LOCALIZED = Object.freeze({
  ko: Object.freeze({
    purpose: '대학생이 생성형 AI를 마케팅 전략에 적용하고 고객, 브랜드, 검증 및 위험 기준으로 AI 마케팅 콘텐츠를 설계하도록 돕습니다.',
    objectives: ['생성형 AI 마케팅의 고객 세분화와 콘텐츠 생성 활용 사례를 식별한다', '프롬프트 설계가 브랜드 톤과 목표 고객 적합성에 미치는 영향을 비교한다', '고객 여정과 마케팅 퍼널에 맞는 AI 마케팅 콘텐츠를 설계한다', 'AI 결과물의 사실 확인, 개인정보, 저작권, 편향 위험을 평가한다'],
    outcomes: ['각 팀은 목표 고객, 고객 여정, 프롬프트, AI 콘텐츠, 사실 확인 기록, 위험 검토와 수정안을 포함한 AI 마케팅 패키지를 완성한다.'],
    core: [
      { title: '생성형 AI 마케팅의 역할과 경계', explanation: '생성형 AI는 고객 분석, 콘텐츠 생성과 변형을 지원하지만 마케팅 전략과 최종 책임은 사람이 맡아야 합니다.', keyPoints: ['마케팅 목표와 성과 지표를 먼저 설정하기', 'AI 결과물을 시장 근거로 오인하지 않기', '인간 검토와 승인 유지하기'] },
      { title: '고객 세분화, 고객 여정과 마케팅 퍼널', explanation: '목표 고객의 요구와 고객 여정 단계에 따라 메시지, 채널과 전환 목표를 다르게 설계합니다.', keyPoints: ['목표 고객의 요구와 장애물 분석', '고객 여정 단계별 콘텐츠 연결', '전환율과 KPI로 성과 측정'] },
      { title: '프롬프트 설계와 브랜드 보이스', explanation: '프롬프트에 고객, 목표, 브랜드 톤, 사실 제한과 출력 형식을 명시하여 콘텐츠 생성을 통제합니다.', keyPoints: ['브랜드 일관성 기준 명시', '개인정보 입력 금지', '불확실한 주장의 사실 확인 요구'] },
      { title: '검증, 윤리와 위험 관리', explanation: 'AI 결과물 검증에는 팩트체크, 개인정보, 저작권, 편향과 과도한 의존 검토가 포함됩니다.', keyPoints: ['허위 정보와 환각 확인', '저작권과 자료 출처 확인', '편향과 차별 점검'] },
    ],
    activity: { title: 'AI 마케팅 콘텐츠 팀 설계', purpose: '조별 활동으로 목표 고객과 고객 여정에 맞는 AI 마케팅 콘텐츠를 만들고 검증합니다.', groupFormat: '3–4인 조별 활동', steps: ['목표 고객, 마케팅 목표와 고객 여정 단계를 정한다.', '브랜드 톤, 제한과 형식을 포함한 프롬프트를 설계한다.', 'AI 콘텐츠 초안을 만들고 사실 확인이 필요한 주장을 표시한다.', '브랜드 일관성, 개인정보, 저작권, 편향과 위험을 검증한다.', '피드백을 반영해 프롬프트와 콘텐츠를 수정한다.'], teacherGuidance: '각 팀에 고객 근거, 프롬프트 제한, 팩트체크와 위험 대응을 설명하도록 요구합니다.', learnerOutput: '고객 분석, 프롬프트, AI 콘텐츠, 검증 체크리스트와 수정안', completionCriteria: ['목표 고객과 마케팅 전략이 명확함', '브랜드 보이스가 일관됨', 'AI 결과물 검증을 완료함', '네 가지 이상의 위험을 식별함'] },
    assessment: { method: '팀 결과물 발표와 평가 기준표', criteria: ['목표 고객 적합성', '브랜드 톤과 브랜드 일관성', '프롬프트 설계 품질', 'AI 결과물의 사실 확인과 콘텐츠 검증', '마케팅 전략과 성과 지표의 타당성', 'AI 위험 관리와 수정 완성도'], feedbackMethod: '동료와 교사가 평가 기준에 따라 구체적인 수정 피드백을 제공합니다.' },
    risks: ['환각과 허위 정보: 모든 제품 주장과 수치를 팩트체크한다.', '개인정보와 프라이버시: 고객 개인 데이터나 민감 정보를 입력하지 않는다.', '저작권과 자료 출처: 콘텐츠 라이선스와 출처 표시를 확인한다.', '편향과 차별: 특정 고객 집단에 대한 편향과 부적절한 표현을 점검한다.', '브랜드 톤: 브랜드 보이스와 브랜드 일관성을 사람이 검토한다.', '과도한 의존: AI 결과물은 인간 검토와 최종 승인을 거친다.'],
  }),
  en: Object.freeze({
    purpose: 'Enable university students to apply generative AI to marketing strategy and design AI marketing content using audience, brand, validation, and risk criteria.',
    objectives: ['Identify uses of generative AI marketing for customer segmentation and content creation', 'Compare how prompt design affects brand voice and target-audience fit', 'Design AI marketing content for a customer journey and marketing funnel stage', 'Evaluate AI output for factual accuracy, privacy, copyright, bias, and over-reliance risks'],
    outcomes: ['Each group completes an AI marketing package containing a target-audience profile, customer journey stage, prompt, AI content, fact-check record, risk review, and revision.'],
    core: [
      { title: 'The role and boundaries of generative AI marketing', explanation: 'Generative AI can support audience analysis and content creation, while people remain accountable for marketing strategy, evidence, and final approval.', keyPoints: ['Set the marketing objective and performance metrics first', 'Do not treat AI output as market evidence', 'Maintain human review and approval'] },
      { title: 'Customer segmentation, customer journey, and marketing funnel', explanation: 'Match messages, channels, and conversion goals to target-audience needs and the relevant customer journey stage.', keyPoints: ['Analyze audience needs and barriers', 'Map content to each funnel stage', 'Measure performance with conversion rate and KPI evidence'] },
      { title: 'Prompt design and brand voice', explanation: 'Specify the audience, objective, brand voice, factual constraints, and output format so content generation remains controlled.', keyPoints: ['State brand consistency criteria', 'Exclude personal data', 'Require fact-checking for uncertain claims'] },
      { title: 'Output validation, ethics, and risk management', explanation: 'AI output validation covers misinformation, privacy, copyright, bias, and over-reliance before publication.', keyPoints: ['Check hallucinations and misinformation', 'Verify copyright and source attribution', 'Review bias and discrimination'] },
    ],
    activity: { title: 'AI marketing content team design', purpose: 'A small-group activity to create and validate AI marketing content for a target audience and customer journey stage.', groupFormat: 'Teams of 3–4', steps: ['Define the target audience, marketing objective, and customer journey stage.', 'Design a prompt with brand voice, constraints, and output format.', 'Create an AI content draft and mark claims requiring fact-checking.', 'Validate brand consistency, privacy, copyright, bias, and other risks.', 'Revise the prompt and content using evidence and feedback.'], teacherGuidance: 'Require each team to explain its audience evidence, prompt constraints, fact-checking, and risk response.', learnerOutput: 'Audience analysis, prompt, AI content, validation checklist, and revised version', completionCriteria: ['Target audience and marketing strategy are explicit', 'Brand voice is consistent', 'AI output validation is documented', 'At least four risks are identified'] },
    assessment: { method: 'Group presentation and analytic rubric', criteria: ['Target-audience fit', 'Brand voice and brand consistency', 'Prompt design quality', 'AI output factual accuracy and content validation', 'Marketing strategy and performance-metric feasibility', 'AI risk management and revision completeness'], feedbackMethod: 'Peers and the teacher give criterion-referenced revision feedback.' },
    risks: ['Hallucination and misinformation: fact-check every product claim and statistic.', 'Privacy and personal data: never enter customer records or sensitive data.', 'Copyright and source attribution: verify content licensing and attribution.', 'Bias and discrimination: review harmful representation across audience groups.', 'Brand voice: use human review to confirm brand consistency.', 'Over-reliance: require human oversight and final approval for every AI output.'],
  }),
});

function localizedAiMarketingCourseContent(payload, requirements) {
  const locale = normalizeLocale(requirements.language);
  if (locale === 'zh') return aiMarketingCourseContent(payload, requirements);
  const base = generalCourseContent(payload, requirements);
  const topical = AI_MARKETING_LOCALIZED[locale];
  return {
    ...base,
    overview: { ...base.overview, courseTitle: requirements.exactTitle, topic: requirements.topic, purpose: topical.purpose, targetAudience: requirements.targetAudience, durationMinutes: requirements.durationMinutes, language: locale, format: requirements.format },
    learningObjectives: [...topical.objectives],
    learningOutcomes: [...topical.outcomes],
    coreContent: structuredClone(topical.core),
    activities: [{ ...structuredClone(topical.activity), durationMinutes: base.sessionPlan.at(-1).durationMinutes }],
    assessment: structuredClone(topical.assessment),
    risksAndNotes: [...topical.risks],
  };
}

export function generateCourseContent(payload, requirements = extractStructuredRequirements('course', payload)) {
  if (isAiMarketing(requirements)) return localizedAiMarketingCourseContent(payload, requirements);
  return generalCourseContent(payload, requirements);
}
...
function semanticRelevanceResult(conceptIds, bodyText, locale) {
  const result = matchAnySemanticConcept(bodyText, conceptIds, locale);
  const score = conceptIds?.length ? result.matchedConcepts.length / conceptIds.length : 1;
  return {
    score: Number(score.toFixed(3)), threshold: 0.65, valid: score >= 0.65,
    locale: result.locale, requiredConcepts: conceptIds || [], matchedConcepts: result.matchedConcepts,
...
export function validateStructuredContent(draftType, content, { allowedUrls = [], requirements, sourcePrompt = '' } = {}) {
  ...
  const locale = normalizeLocale(requirements?.language || content?.overview?.language);
  ...
  const objectiveVerbMatches = course && Array.isArray(content?.learningObjectives)
    ? content.learningObjectives.map((objective) => ({ objectiveIndex: content.learningObjectives.indexOf(objective), ...matchMeasurableObjectiveVerb(objective, locale) }))
    : [];
  if (course && objectiveVerbMatches.some((match) => !match.valid)) errors.push('Learning objectives must use measurable verbs.');
  ...
  const topicRelevance = requirements?.subjectConcepts?.length
    ? semanticRelevanceResult(requirements.subjectConcepts, bodyText, locale)
    : relevanceResult(requirements?.subjectKeywords || [], bodyText);
  ...
  const aiMarketing = requirements?.subjectConcepts?.includes('GENERATIVE_AI') && requirements.subjectConcepts.includes('MARKETING_STRATEGY');
  let aiMarketingDiagnostics = null;
  if (course && aiMarketing) {
    const objectiveText = (content?.learningObjectives || []).join(' ');
    const objectiveConcepts = matchAnySemanticConcept(objectiveText, AI_MARKETING_TOPIC_CONCEPTS, locale);
    if (objectiveConcepts.matchedConcepts.length < 4) errors.push('Learning objectives are not specific enough for generative AI marketing.');
    const activityMatches = (content?.activities || []).map((activity) => ({
      group: matchSemanticConcept(activity.groupFormat || '', 'GROUP_ACTIVITY', locale),
      concepts: matchAnySemanticConcept(strings(activity).join(' '), AI_MARKETING_ACTIVITY_CONCEPTS, locale),
    }));
    const relevantActivity = activityMatches.some((match) => match.group.matched && match.concepts.matchedConcepts.length >= 3);
    ...
    const assessmentConcepts = matchAnySemanticConcept(assessmentText, AI_MARKETING_ASSESSMENT_CONCEPTS, locale);
    if (assessmentConcepts.matchedConcepts.length < 5) errors.push('Assessment criteria are not specific enough.');
    const riskText = (content?.risksAndNotes || []).join(' ');
    const riskConcepts = matchAnySemanticConcept(riskText, AI_RISK_CONCEPTS, locale);
    if (riskConcepts.matchedConcepts.length < 4) errors.push('At least four AI risk categories are required.');
    const genericTemplate = matchGenericTemplateIndicators(bodyText, locale);
    if (genericTemplate.matched) errors.push('Generic template language is not acceptable for this topic.');
    aiMarketingDiagnostics = Object.freeze({
      objectives: objectiveConcepts,
      activity: Object.freeze({ valid: relevantActivity, candidates: Object.freeze(activityMatches) }),
      assessment: assessmentConcepts,
      risks: riskConcepts,
      genericTemplate,
    });
  }
  ...
  return {
    ...
    locale,
    semanticDiagnostics: Object.freeze({ topic: topicRelevance, aiMarketing: aiMarketingDiagnostics }),
    objectiveVerbMatches: Object.freeze(objectiveVerbMatches),
    thresholds: Object.freeze({ topicRelevance: 0.65, promptOverlap: 0.35, aiMarketingObjectives: 4, groupActivityConcepts: 3, assessmentConcepts: 5, riskConcepts: 4 }),
    validatorVersion: 'locale-semantic-v1',
  };
}
...
export function getXchangeSectionMarkers(draftType, content) {
  const h = HEADINGS[content?.overview?.language] || HEADINGS.en;
  const sections = draftType === 'course'
    ? [['overview', null], ['learningObjectives', h.objectives], ['learningOutcomes', h.outcomes], ['sessionPlan', h.plan], ['coreContent', h.core], ['activities', h.activities], ['discussionQuestions', h.questions], ['assessment', h.assessment], ['resources', h.resources], ['risksAndNotes', h.risks], ['extension', h.extension]]
    : [['overview', null], ['learningOutcomes', h.outcomes], ['materials', h.materials], ['preparation', h.preparation], ['steps', h.steps], ['teacherScript', h.script], ['discussionQuestions', h.questions], ['expectedOutput', h.expected], ['assessmentCriteria', h.assessment], ['differentiation', h.differentiation], ['closing', h.closing]];
  return sections.map(([section, headingText]) => ({ section, headingText }));
}

export function buildXchangeNotionBlocks(draftType, content) {
  return draftType === 'course' ? buildCourseNotionBlocks(content) : buildLearningActivityNotionBlocks(content);
}
