...
export async function createXchangeNotionDraft({ draftType, payload, targetDataSource = TARGET, env = process.env, notionClient, logger = console.error } = {}) {
  ...
  try {
    ...
    if (['NOTION_INVALID_RESPONSE', 'DATA_SOURCE_NOT_ALLOWED'].includes(error?.code)) throw error;
