...
export async function createXchangeNotionDraft({ draftType, payload, content, targetDataSource = TARGET, env = process.env, notionClient, logger = console.error } = {}) {
  ...
  try {
    ...
    const quality = validateStructuredContent(draftType, content, { allowedUrls: [payload.fileUrl, payload.materialsUrl].filter(Boolean) });
