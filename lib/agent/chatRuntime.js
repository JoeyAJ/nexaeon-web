...
export function isAgentEnabled(env = process.env) {
  return env.NEXAEON_AGENT_ENABLED === 'true' || env.NEXON_AGENT_ENABLED === 'true';
}
...
export function buildResponsesApiRequest({ query, lang, history, numberedSources, model, queryIntent }) {
  return {
...
export async function createGroundedAnswer({ openai, query, lang, history, numberedSources, queryIntent }) {
...
export async function handleAgentChatRequest(req, res, deps = {}) {
  ...
  try {
    ...
    logAgentChatEvent('input_moderation_unavailable');
    sendJson(req, res, createModelFailureResponse({
  ...
  try {
    ...
    if (!validatedOutput.ok) {
      logAgentChatEvent('model_output_invalid');
      sendJson(req, res, createModelFailureResponse({
        reason: 'model_unavailable',
        citations: deterministicCitations,
        partialSources,
    ...
    try {
      ...
      logAgentChatEvent('output_moderation_unavailable');
      sendJson(req, res, createModelFailureResponse({
        reason: 'model_unavailable',
        citations: deterministicCitations,
        partialSources,
    ...
    logAgentChatEvent(timeout ? 'model_timeout' : 'model_request_failed');
    sendJson(req, res, createModelFailureResponse({
      reason,
      citations: deterministicCitations,
      partialSources,
