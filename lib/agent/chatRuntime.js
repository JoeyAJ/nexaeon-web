...
function looksLikeLanguage(text, lang) {
  if (lang === 'zh') return /[\u3400-\u9fff]/u.test(text) || /\bNexAeon\b|\bJoey\b|\bDemo\b|\bLearning Coaching\b|\bAction Center\b|\bKnowledge Lab\b/u.test(text);
  if (lang === 'ko') return /[가-힣]/u.test(text) || /\bNexAeon\b|\bJoey\b|\bDemo\b|\bLearning Coaching\b|\bAction Center\b|\bKnowledge Lab\b/u.test(text);
  return !/[\u3400-\u9fff가-힣]/u.test(text);
}
...
function getFallbackSuggestionOrder(numberedSources = [], queryIntent = {}) {
  ...
  const fromSources = numberedSources.map((source) => {
    ...
    if (source.moduleKey === 'field-lab' && /collab/i.test(source.itemType || source.moduleLabel || '')) return 'collaboration';
...
export function createFallbackSuggestedQuestions({ query = '', lang = 'en', numberedSources = [], queryIntent = {} } = {}) {
  const fallback = FALLBACK_SUGGESTIONS[lang] || FALLBACK_SUGGESTIONS.en;
...
export function validateSuggestedQuestions({ suggestions = [], query = '', lang = 'en', numberedSources = [], queryIntent = {} } = {}) {
  ...
  for (const suggestion of uniqueCompactArray(suggestions).slice(0, 6)) {
    ...
    if (!looksLikeLanguage(text, lang)) continue;
    ...
    const related = !numberedSources.length
      ...
      || /研究|學習|知識|合作|行動|身份|데모|연구|학습|지식|협력|액션|정체성/u.test(text);
    if (!related) continue;
    seen.add(key);
    validated.push(text);
  ...
  const fallback = createFallbackSuggestedQuestions({ query, lang, numberedSources, queryIntent });
...
export function buildDeveloperInstruction(lang) {
  ...
  return [
    ...
    'Return at most three suggested questions in the same language. They must be answerable from the supplied public NexAeon modules and must not ask for private data, web search, account access, writes, email, calendar, or files.',
