...
export function validateChatRequestBody(body) {
  ...
  const langInput = String(body.lang || '').trim();
...
function buildModelInput({ query, lang, history, numberedSources, queryIntent }) {
...
export function buildResponsesApiRequest({ query, lang, history, numberedSources, model, queryIntent, maxOutputTokens = 800 }) {
  return {
    ...
    input: [{
      role: 'user',
      content: [{ type: 'input_text', text: buildModelInput({ query, lang, history, numberedSources, queryIntent }) }],
    }],
    text: {
      format: {
        type: 'json_schema',
