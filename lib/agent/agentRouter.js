...
const EXPLICIT_PATTERNS = Object.freeze({
  identity: [/identity agent/u, /身份\s*agent/u, /身分\s*agent/u, /아이덴티티\s*agent/u, /정체성\s*(?:agent|에이전트)/u],
  research: [/research agent/u, /研究\s*agent/u, /리서치\s*agent/u, /연구\s*(?:agent|에이전트)/u, /nexaeon explorer/u],
  coaching: [/coaching agent/u, /教練\s*agent/u, /코칭\s*agent/u, /학습\s*코칭\s*(?:agent|에이전트)/u, /nexaeon xchange/u],
  knowledge: [/knowledge agent/u, /知識\s*agent/u, /지식\s*(?:agent|에이전트)/u, /nexaeon archivist/u],
  prototype: [/prototype agent/u, /原型\s*agent/u, /프로토타입\s*(?:agent|에이전트)/u, /nexaeon engineer/u],
  action: [/action agent/u, /行動\s*agent/u, /액션\s*agent/u, /실행\s*(?:agent|에이전트)/u, /nexaeon orchestrator/u],
});
...
export function routeAgentRequest({ query, currentRoute = '', currentModule = '', preferredAgent = '', history = [] } = {}) {
  ...
  const preferredHint = MODULE_AGENT_IDS.includes(preferredAgent) ? preferredAgent : null;
  ...
  const intentAgent = top.score >= 4 ? top.id : null;
  const primaryAgent = explicit || crossModuleRule?.primary || intentAgent || preferredHint || routeHint || null;
  if (!text || !primaryAgent) {
    return { primaryAgent: null, supportingAgents: [], confidence: 0.25, reasonCode: 'navigator_fallback', contextSources: [], requiresClarification: false };
  }
  ...
  const confidence = explicit
    ? 0.99
    : intentAgent || crossModuleRule
      ? Math.min(0.96, 0.58 + intentScore * 0.035 + (routeHint === primaryAgent ? 0.05 : 0))
      : preferredHint
        ? 0.54
        : 0.46;
  ...
  if (preferredHint) contextSources.push('preferredAgent');
  if (routeHint) contextSources.push(currentModule ? 'currentModule' : 'currentRoute');
  if (conversationMatches.length) contextSources.push('conversation');
  const reasonCode = explicit
    ? 'explicit_agent'
    : intentAgent || crossModuleRule
      ? `${primaryAgent}_intent`
      : preferredHint
        ? 'preferred_agent'
        : 'module_context';
  return {
