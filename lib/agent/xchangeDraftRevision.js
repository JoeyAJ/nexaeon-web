...
export function validateXchangeRevisionRequest(body, draftType) {
  ...
  if (editMode === 'edit_section' && !instruction) fail('INSTRUCTION_REQUIRED');
...
function instructionGeneratedSection({ draftType, targetPath, instruction, normalizedPayload, requirements }) {
  ...
  const languageValue = Array.isArray(normalizedPayload.language) ? normalizedPayload.language[0] : normalizedPayload.language;
  const language = /^(?:zh|繁體中文|简体中文|簡體中文|中文)$/iu.test(languageValue)
    ? 'zh'
    : /^(?:ko|한국어|韓國語|韩语)$/iu.test(languageValue) ? 'ko' : 'en';
  ...
  const brandObjective = language === 'zh'
    ? `評估「${topic}」成果，完成品牌一致性評估並提出具體修訂建議`
    : language === 'ko'
...
export function reviseXchangeDraft({ sourcePreview, edit, normalizePayload }) {
  ...
  if (edit.editMode === 'edit_field') {
    ...
    const replacementIsUnchanged = edit.replacementValue !== undefined
      && !changed(sourcePreview.contentPreview[edit.targetPath], edit.replacementValue);
    content[edit.targetPath] = edit.replacementValue === undefined || replacementIsUnchanged
