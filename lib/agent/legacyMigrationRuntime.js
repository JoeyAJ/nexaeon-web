...
function inventory(projects, audits) {
  ...
  const migratedActionAudits = audits.filter((audit) => audit.recordType === 'migrated' && audit.schemaVersion === 'v1' && audit.toolId === 'createActionDraft');
  return { invalidAudits, legacyAuditCandidates, legacyDrafts, alreadyMigratedAudits, alreadyMigratedDrafts, auditsToCreate, migratedActionAudits };
}
...
function uniqueLifecycleCandidate(draft, candidates) {
  const byAction = candidates.filter((candidate) => String(candidate.externalRecordId || candidate.legacy?.payload?.externalRecordId || '') === draft.sourceRecordId);
  const byIdempotency = candidates.filter((candidate) => String(candidate.idempotencyKey || candidate.legacy?.payload?.idempotencyKey || '') === draft.fields['Idempotency Key']);
  let matches = byAction.length ? byAction : byIdempotency;
  if (matches.length > 1) {
    const succeeded = matches.filter((candidate) => String(candidate.executionStatus || candidate.legacy?.payload?.executionStatus || '') === 'succeeded');
    if (succeeded.length === 1) matches = succeeded;
  }
...
function migrationWritePlan({ found, batchId, payloadHash = 'preflight', actor, fingerprint, now, schema }) {
  ...
  return [
    ...
    ...found.legacyDrafts.map((draft) => {
      const candidate = uniqueLifecycleCandidate(draft, [...found.migratedActionAudits, ...found.auditsToCreate.map((legacy) => ({ legacy }))]);
      const fields = { ...draft.fields };
      if (candidate) {
        fields['Audit Record ID'] = candidate.id || 'recPreflightAudit000000';
        if (fields['Operation ID'] === 'Unknown') fields['Operation ID'] = String(candidate.operationId || candidate.legacy?.payload?.operationId || 'Unknown').slice(0, 80);
      }
      return { kind: 'legacy-draft-update', tableRole: 'projects', sourceRecordId: draft.sourceRecordId, fields: prepare('projects', fields) };
    }),
...
export async function executeLegacyMigration({ body, actor, req, env = process.env, now = Date.now(), dataSource = createAirtableMigrationDataSource({ env }) }) {
  ...
  const migratedTargets = [...found.migratedActionAudits];
  for (const legacy of found.auditsToCreate) {
    try {
      ...
      migratedTargets.push({ id, legacy, externalRecordId: legacy.payload?.externalRecordId, idempotencyKey: legacy.payload?.idempotencyKey, operationId: legacy.payload?.operationId, executionStatus: legacy.payload?.executionStatus });
  ...
  for (const draft of found.legacyDrafts) {
    try {
      ...
      const fields = { ...plannedByKind.get(`legacy-draft-update:${draft.sourceRecordId}`) };
      const candidate = uniqueLifecycleCandidate(draft, migratedTargets);
      if (candidate) {
        fields['Audit Record ID'] = candidate.id;
        if (fields['Operation ID'] === 'Unknown') fields['Operation ID'] = String(candidate.operationId || 'Unknown').slice(0, 80);
      } else if (fields['Audit Record ID'] === 'recPreflightAudit000000') delete fields['Audit Record ID'];
      await dataSource.updateProject(draft.sourceRecordId, fields);
