...
export function checkActionAuditConsistency({ projects = [], audits = [] } = {}) {
  const actions = projects.filter((record) => value(record.fields, 'Action Draft Schema Version') === 'v1'
    || value(record.fields, 'Project Name').startsWith('[Draft '));
  const auditById = new Map(audits.map((audit) => [audit.id, audit]));
  const formalAudits = audits.filter((audit) => audit.schemaVersion === 'v1' && audit.toolId === 'createActionDraft');
  ...
  for (const action of actions) {
    ...
    if (value(fields, 'Action Draft Schema Version') !== 'v1') {
      results.push(issue('legacy', action, null));
      continue;
    }
    ...
    const auditRecordId = value(fields, 'Audit Record ID');
    if (!auditRecordId) {
  ...
  for (const audit of formalAudits.filter((candidate) => candidate.externalRecordId || candidate.executionStatus === 'succeeded')) {
    ...
    const action = projects.find((record) => record.id === audit.externalRecordId);
  ...
  const counts = Object.fromEntries(['consistent', 'action-missing-audit', 'audit-missing-action', 'link-mismatch', 'operation-mismatch', 'idempotency-mismatch', 'duplicate', 'legacy', 'unknown'].map((category) => [category, results.filter((item) => item.category === category).length]));
