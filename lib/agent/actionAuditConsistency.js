...
function normalizedTokens(input) {
  const generic = new Set(['unknown', 'legacy', 'migration', 'createactiondraft', 'production', 'acceptance', 'action', 'formal', 'schema', 'created', 'execute']);
  return new Set(safeText(input, 6000).toLowerCase().split(/[^\p{L}\p{N}_-]+/u).filter((token) => token.length >= 6 && !generic.has(token)).slice(0, 120));
}
...
export function findForensicAuditCandidates(action, audits = []) {
  ...
  }).filter(({ matchScore, evidence }) => {
    const exactIdentity = evidence.some(({ field }) => ['External Record ID', 'Idempotency Key', 'Operation ID'].includes(field));
    const nearInTime = evidence.some(({ field, match }) => field === 'Created timestamp' && ['within-5-minutes', 'within-30-minutes'].includes(match));
    const distinctivePayload = evidence.some(({ field, weight }) => field === 'Sanitized payload fingerprint' && weight >= 15);
    return exactIdentity || (nearInTime && matchScore >= 30) || (distinctivePayload && matchScore >= 45);
  })
