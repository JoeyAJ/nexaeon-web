...
export function checkActionAuditConsistency({ projects = [], audits = [] } = {}) {
  ...
  for (const action of actions) {
    ...
    if (!audit.externalRecordId) {
      ...
      if (fieldsMatch && succeededSibling) {
        linkedAudits.add(succeededSibling.id);
        results.push(issue('consistent', action, audit, { reason: 'linked-preview-with-succeeded-lifecycle', lifecycleAuditRecordIds: [audit.id, succeededSibling.id], expectedAuditRecordId: audit.id, currentAuditRecordId: audit.id })); continue;
      }
