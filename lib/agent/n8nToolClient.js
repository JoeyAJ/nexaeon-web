  ...
  return Object.freeze({
    async execute({ requestId, traceId, agentId, toolId, taskType, input, idempotencyKey, authority }) {
      ...
      try {
        ...
        auditCollector.record({ requestId, traceId, agentId, toolId, taskType, status: 'failed', durationMs: Date.now() - startedAt, errorCode: normalized.code, externalExecutionId: null });
        throw normalized;
      }
    },
  });
