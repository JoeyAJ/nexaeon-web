...
const COPY = {
  zh: { title: '管理員稽核紀錄', subtitle: '僅限已授權管理員。紀錄為 append-only，敏感資訊已在伺服器端移除。', actor: '管理員 ID', code: '存取碼', login: '驗證管理員', logout: '登出', refresh: '重新整理', from: '起始日期', to: '結束日期', agent: 'Agent', tool: '工具', status: '狀態', all: '全部', empty: '沒有符合條件的稽核紀錄。', failed: '無法載入', back: '返回首頁' },
  ko: { title: '관리자 감사 로그', subtitle: '승인된 관리자 전용입니다. 로그는 append-only이며 민감한 정보는 서버에서 제거됩니다.', actor: '관리자 ID', code: '접근 코드', login: '관리자 확인', logout: '로그아웃', refresh: '새로고침', from: '시작일', to: '종료일', agent: 'Agent', tool: '도구', status: '상태', all: '전체', empty: '조건에 맞는 감사 로그가 없습니다.', failed: '불러오기 실패', back: '홈으로' },
  en: { title: 'Admin audit log', subtitle: 'Authorized administrators only. Records are append-only and secrets are removed server-side.', actor: 'Admin ID', code: 'Access code', login: 'Verify admin', logout: 'Sign out', refresh: 'Refresh', from: 'Date from', to: 'Date to', agent: 'Agent', tool: 'Tool', status: 'Status', all: 'All', empty: 'No audit records match these filters.', failed: 'Unable to load', back: 'Back home' },
};
...
export default function AdminAuditPage({ lang, setLang, navigate }) {
  ...
  const [filters, setFilters] = useState({ dateFrom: '', dateTo: '', agentId: '', toolId: '', executionStatus: '' });
    ...
    <main className="admin-audit-page" data-testid="admin-audit-page">
      ...
      {auth.phase !== 'authenticated' ? (
        ...
        <>
          ...
          <form className="admin-audit-filters" onSubmit={(event) => { event.preventDefault(); loadRecords(); }}><label>{copy.from}<input type="date" value={filters.dateFrom} onChange={(event) => setFilters((current) => ({ ...current, dateFrom: event.target.value }))} /></label><label>{copy.to}<input type="date" value={filters.dateTo} onChange={(event) => setFilters((current) => ({ ...current, dateTo: event.target.value }))} /></label><label>{copy.agent}<select value={filters.agentId} onChange={(event) => setFilters((current) => ({ ...current, agentId: event.target.value }))}><option value="">{copy.all}</option><option value="orchestrator">orchestrator</option></select></label><label>{copy.tool}<select value={filters.toolId} onChange={(event) => setFilters((current) => ({ ...current, toolId: event.target.value }))}><option value="">{copy.all}</option><option value="createActionDraft">createActionDraft</option></select></label><label>{copy.status}<select value={filters.executionStatus} onChange={(event) => setFilters((current) => ({ ...current, executionStatus: event.target.value }))}><option value="">{copy.all}</option>{['previewed', 'executing', 'succeeded', 'failed', 'expired', 'cancelled'].map((status) => <option key={status} value={status}>{status}</option>)}</select></label><button type="submit" disabled={loading}>{copy.refresh}</button></form>
          ...
          <div className="admin-audit-list">{records.map((record) => <article key={record.auditId}><div><strong>{record.executionStatus}</strong><time>{record.timestamp}</time></div><dl><div><dt>operation</dt><dd>{record.operationId}</dd></div><div><dt>actor</dt><dd>{record.actorId} · {record.actorRole}</dd></div><div><dt>agent / tool</dt><dd>{record.agentId} / {record.toolId}</dd></div><div><dt>target</dt><dd>{record.targetDataSource}</dd></div><div><dt>record</dt><dd>{record.externalRecordId || '—'}</dd></div><div><dt>error</dt><dd>{record.errorCode || '—'}</dd></div><div><dt>duration</dt><dd>{record.duration} ms</dd></div></dl></article>)}</div>
        </>
      )}
    </main>
  );
