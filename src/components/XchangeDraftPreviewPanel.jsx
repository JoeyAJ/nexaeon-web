...
const COPY = Object.freeze({
  zh: {
    ...
    signOut: '登出', adminRequired: '需要管理員登入才能建立正式 Preview Audit。', preview: '建立 Preview', previewing: 'AI 正在生成並驗證課程 Preview……', generation: '生成來源', fallback: '已使用安全 fallback',
    ...
    validateDraft: '驗證已建立草稿', validatingDraft: '正在唯讀驗證草稿……', validationTitle: 'Delivery Readiness', validationFailed: '驗證失敗', readOnlyValidation: '此驗證為唯讀操作，不會修改、發布或刪除 Notion 頁面。', propertiesStatus: 'Properties', contentStatus: '正文完整性', revisionStatus: 'Revision 一致性', durationStatus: '時間', safetyValidation: '安全狀態', blockCounts: 'Block 數量', missingSections: '缺失區段', mismatches: '不一致項目', validationWarnings: '警告', validationWrites: 'Validation writes',
  },
  ko: {
    ...
    signOut: '로그아웃', adminRequired: '정식 Preview Audit을 만들려면 관리자 로그인이 필요합니다.', preview: 'Preview 만들기', previewing: 'AI가 과정 Preview를 생성하고 검증하는 중…', generation: '생성 소스', fallback: '안전 fallback 사용됨',
    ...
    validateDraft: '생성된 초안 검증', validatingDraft: '초안을 읽기 전용으로 검증하는 중…', validationTitle: 'Delivery Readiness', validationFailed: '검증 실패', readOnlyValidation: '이 검증은 읽기 전용이며 Notion 페이지를 수정, 게시 또는 삭제하지 않습니다.', propertiesStatus: 'Properties', contentStatus: '본문 완전성', revisionStatus: 'Revision 일관성', durationStatus: '시간', safetyValidation: '안전 상태', blockCounts: 'Block 수', missingSections: '누락 섹션', mismatches: '불일치 항목', validationWarnings: '경고', validationWrites: 'Validation writes',
  },
  en: {
    ...
    signOut: 'Sign out', adminRequired: 'Admin sign-in is required to create the formal Preview Audit.', preview: 'Create Preview', previewing: 'AI is generating and validating the course Preview…', generation: 'Generation source', fallback: 'Safe fallback used',
    ...
    validateDraft: 'Validate created draft', validatingDraft: 'Validating the draft in read-only mode…', validationTitle: 'Delivery Readiness', validationFailed: 'Validation failed', readOnlyValidation: 'This validation is read-only and does not modify, publish, or delete the Notion page.', propertiesStatus: 'Properties', contentStatus: 'Content integrity', revisionStatus: 'Revision consistency', durationStatus: 'Duration', safetyValidation: 'Safety status', blockCounts: 'Block counts', missingSections: 'Missing sections', mismatches: 'Mismatches', validationWarnings: 'Warnings', validationWrites: 'Validation writes',
  },
});
...
const ERROR_COPY = Object.freeze({
  zh: {
    AUTH_REQUIRED: '需要管理員登入', AUTH_SESSION_EXPIRED: '管理員 session 已過期', AUTH_ROLE_FORBIDDEN: '此帳號沒有管理員權限', CSRF_INVALID: '安全驗證已失效，請重新登入', ORIGIN_NOT_ALLOWED: '此來源不允許執行操作', OPERATION_NOT_FOUND: '找不到 Operation', PREVIEW_NOT_FOUND: '找不到有效 Preview', PREVIEW_EXPIRED: 'Preview 已過期，請重新建立', PREVIEW_ALREADY_EXECUTED: '此 Preview 已執行，不能再次使用', CONFIRMATION_REQUIRED: '請先勾選明確確認', CONFIRMATION_INVALID: '確認憑證無效', CONFIRMATION_MISMATCH: '表單或 Preview 已改變，請重新建立 Preview', CONFIRMATION_REQUESTER_MISMATCH: '此 Preview 不屬於目前的管理員 session', TOOL_NOT_ALLOWED: '此工具不允許寫入', DATA_SOURCE_NOT_ALLOWED: '目標資料來源不允許寫入', MASS_ASSIGNMENT_REJECTED: '請求包含不允許的欄位', CONTENT_VALIDATION_FAILED: '生成內容未通過品質驗證，未建立草稿', MODEL_DISABLED: '模型生成目前停用', MODEL_CONFIGURATION_MISSING: '模型 Provider 尚未完成設定', MODEL_CONFIGURATION_INVALID: '模型 Provider 設定無效', MODEL_PROVIDER_UNAVAILABLE: '模型 Provider 暫時無法使用', MODEL_PROVIDER_ERROR: '模型 Provider 暫時無法使用', MODEL_TIMEOUT: '模型生成逾時，請重新生成', MODEL_RATE_LIMITED: '模型服務忙碌中，請稍後重新生成', MODEL_JSON_INVALID: '模型回傳的 JSON 無法解析', MODEL_SCHEMA_INVALID: '模型回傳內容未通過結構驗證', INVALID_EDIT_MODE: '修訂模式無效', EDIT_TARGET_NOT_ALLOWED: '此欄位或區段不可修改', REPLACEMENT_REQUIRED: '請輸入替代值', INSTRUCTION_REQUIRED: '請輸入修改要求', NO_EFFECTIVE_CHANGE: '修改沒有產生有效差異，未建立新 Preview', SCHEMA_MISMATCH: 'Learning Coaching schema 不符合安全寫入合約', NOTION_CONFIGURATION_MISSING: 'Notion 設定未完成', NOTION_REQUEST_FAILED: 'Notion 建立草稿失敗', NOTION_INVALID_RESPONSE: 'Notion 回傳的頁面資料不完整，無法完成驗證。', AUDIT_PERSISTENCE_FAILED: 'Audit 無法保存，已停止操作', EXECUTION_IN_PROGRESS: '此草稿正在建立，請勿重複送出', VALIDATION_SNAPSHOT_INCOMPLETE: '缺少可信的執行快照，無法完成驗證。', VALIDATION_CANONICALIZATION_FAILED: '驗證資料格式處理失敗，未修改 Notion 頁面。', EXECUTION_NOT_SUCCEEDED: '此 Operation 尚未成功建立草稿', VALIDATION_TARGET_NOT_FOUND: '找不到可驗證的 Xchange 草稿', NOTION_VALIDATION_READ_FAILED: 'Notion 草稿讀取失敗', VALIDATION_LIMIT_EXCEEDED: '草稿內容超過安全驗證限制',
  },
  ko: {
    AUTH_REQUIRED: '관리자 로그인이 필요합니다', AUTH_SESSION_EXPIRED: '관리자 session이 만료되었습니다', AUTH_ROLE_FORBIDDEN: '관리자 권한이 없습니다', CSRF_INVALID: '보안 확인이 만료되었습니다. 다시 로그인해 주세요', ORIGIN_NOT_ALLOWED: '이 출처에서는 작업을 실행할 수 없습니다', OPERATION_NOT_FOUND: 'Operation을 찾을 수 없습니다', PREVIEW_NOT_FOUND: '유효한 Preview를 찾을 수 없습니다', PREVIEW_EXPIRED: 'Preview가 만료되었습니다. 다시 만들어 주세요', PREVIEW_ALREADY_EXECUTED: '이미 실행된 Preview입니다', CONFIRMATION_REQUIRED: '명시적 확인란을 선택해 주세요', CONFIRMATION_INVALID: '확인 토큰이 유효하지 않습니다', CONFIRMATION_MISMATCH: '양식 또는 Preview가 변경되었습니다. Preview를 다시 만들어 주세요', CONFIRMATION_REQUESTER_MISMATCH: '현재 관리자 session의 Preview가 아닙니다', TOOL_NOT_ALLOWED: '이 도구는 쓰기가 허용되지 않습니다', DATA_SOURCE_NOT_ALLOWED: '대상 데이터 소스에 쓸 수 없습니다', MASS_ASSIGNMENT_REJECTED: '허용되지 않은 필드가 포함되어 있습니다', CONTENT_VALIDATION_FAILED: '수정 후 품질 검사를 통과하지 못해 실행할 수 없습니다', INVALID_EDIT_MODE: '수정 모드가 유효하지 않습니다', EDIT_TARGET_NOT_ALLOWED: '이 필드 또는 섹션은 수정할 수 없습니다', REPLACEMENT_REQUIRED: '대체 값을 입력하세요', INSTRUCTION_REQUIRED: '수정 지시를 입력하세요', NO_EFFECTIVE_CHANGE: '수정 결과에 유효한 차이가 없어 새 Preview를 만들지 않았습니다', SCHEMA_MISMATCH: 'Learning Coaching schema가 안전 쓰기 계약과 일치하지 않습니다', NOTION_CONFIGURATION_MISSING: 'Notion 설정이 완료되지 않았습니다', NOTION_REQUEST_FAILED: 'Notion 초안 생성에 실패했습니다', NOTION_INVALID_RESPONSE: 'Notion이 불완전한 페이지 데이터를 반환하여 검증을 완료할 수 없습니다.', AUDIT_PERSISTENCE_FAILED: 'Audit을 저장할 수 없어 작업을 중단했습니다', EXECUTION_IN_PROGRESS: '이 초안을 생성 중입니다. 다시 제출하지 마세요', VALIDATION_SNAPSHOT_INCOMPLETE: '신뢰할 수 있는 실행 스냅샷이 없어 검증을 완료할 수 없습니다.', VALIDATION_CANONICALIZATION_FAILED: '검증 데이터 형식 처리에 실패했으며 Notion 페이지는 수정되지 않았습니다.', EXECUTION_NOT_SUCCEEDED: '이 Operation은 아직 초안을 성공적으로 만들지 않았습니다', VALIDATION_TARGET_NOT_FOUND: '검증할 Xchange 초안을 찾을 수 없습니다', NOTION_VALIDATION_READ_FAILED: 'Notion 초안을 읽지 못했습니다', VALIDATION_LIMIT_EXCEEDED: '초안 내용이 안전 검증 한도를 초과했습니다',
  },
  en: {
    AUTH_REQUIRED: 'Administrator sign-in is required', AUTH_SESSION_EXPIRED: 'The administrator session expired', AUTH_ROLE_FORBIDDEN: 'This account is not an administrator', CSRF_INVALID: 'The security check expired; please sign in again', ORIGIN_NOT_ALLOWED: 'Operations are not allowed from this origin', OPERATION_NOT_FOUND: 'The operation was not found', PREVIEW_NOT_FOUND: 'No valid preview was found', PREVIEW_EXPIRED: 'The preview expired; create a new preview', PREVIEW_ALREADY_EXECUTED: 'This preview has already been executed', CONFIRMATION_REQUIRED: 'Select the explicit confirmation checkbox', CONFIRMATION_INVALID: 'The confirmation token is invalid', CONFIRMATION_MISMATCH: 'The form or preview changed; create a new preview', CONFIRMATION_REQUESTER_MISMATCH: 'This preview belongs to a different administrator session', TOOL_NOT_ALLOWED: 'This tool is not allowed to write', DATA_SOURCE_NOT_ALLOWED: 'This target data source is not allowed', MASS_ASSIGNMENT_REJECTED: 'The request contains a forbidden field', CONTENT_VALIDATION_FAILED: 'The revised content did not pass quality validation and cannot execute', INVALID_EDIT_MODE: 'The revision mode is invalid', EDIT_TARGET_NOT_ALLOWED: 'This field or section cannot be edited', REPLACEMENT_REQUIRED: 'Enter a replacement value', INSTRUCTION_REQUIRED: 'Enter an edit instruction', NO_EFFECTIVE_CHANGE: 'The edit made no effective change, so no new Preview was created', SCHEMA_MISMATCH: 'The Learning Coaching schema does not match the safe write contract', NOTION_CONFIGURATION_MISSING: 'Notion configuration is incomplete', NOTION_REQUEST_FAILED: 'Notion could not create the draft', NOTION_INVALID_RESPONSE: 'Notion returned incomplete page data, so validation could not be completed.', AUDIT_PERSISTENCE_FAILED: 'Audit could not be saved, so the operation stopped', EXECUTION_IN_PROGRESS: 'This draft is already being created; do not submit again', VALIDATION_SNAPSHOT_INCOMPLETE: 'A trustworthy execution snapshot is missing, so validation could not be completed.', VALIDATION_CANONICALIZATION_FAILED: 'Validation data formatting failed; the Notion page was not modified.', EXECUTION_NOT_SUCCEEDED: 'This Operation has not successfully created a draft', VALIDATION_TARGET_NOT_FOUND: 'No valid Xchange draft was found for validation', NOTION_VALIDATION_READ_FAILED: 'The Notion draft could not be read', VALIDATION_LIMIT_EXCEEDED: 'The draft exceeds the safe validation limit',
  },
});

const MODEL_ERROR_COPY = Object.freeze({
  ko: { MODEL_DISABLED: '모델 생성이 비활성화되어 있습니다', MODEL_CONFIGURATION_MISSING: '모델 Provider 설정이 완료되지 않았습니다', MODEL_CONFIGURATION_INVALID: '모델 Provider 설정이 유효하지 않습니다', MODEL_PROVIDER_UNAVAILABLE: '모델 Provider를 일시적으로 사용할 수 없습니다', MODEL_PROVIDER_ERROR: '모델 Provider를 일시적으로 사용할 수 없습니다', MODEL_TIMEOUT: '모델 생성 시간이 초과되었습니다. 다시 생성해 주세요', MODEL_RATE_LIMITED: '모델 서비스가 사용 중입니다. 잠시 후 다시 생성해 주세요', MODEL_JSON_INVALID: '모델이 반환한 JSON을 해석할 수 없습니다', MODEL_SCHEMA_INVALID: '모델 결과가 구조 검증을 통과하지 못했습니다' },
  en: { MODEL_DISABLED: 'Model generation is disabled', MODEL_CONFIGURATION_MISSING: 'The model Provider is not configured', MODEL_CONFIGURATION_INVALID: 'The model Provider configuration is invalid', MODEL_PROVIDER_UNAVAILABLE: 'The model Provider is temporarily unavailable', MODEL_PROVIDER_ERROR: 'The model Provider is temporarily unavailable', MODEL_TIMEOUT: 'Model generation timed out; regenerate the draft', MODEL_RATE_LIMITED: 'The model service is busy; regenerate shortly', MODEL_JSON_INVALID: 'The model returned invalid JSON', MODEL_SCHEMA_INVALID: 'The model output failed schema validation' },
});

function errorText(lang, code) {
  return COPY[lang]?.[code] || ERROR_COPY[lang]?.[code] || MODEL_ERROR_COPY[lang]?.[code] || COPY.en[code] || ERROR_COPY.en[code] || MODEL_ERROR_COPY.en[code] || String(code || 'REQUEST_FAILED');
}
...
export default function XchangeDraftPreviewPanel({ lang }) {
  ...
  const [validation, setValidation] = useState({ phase: 'idle', result: null, errorCode: '' });
  ...
  async function logout() {
    try {
      ...
      setValidation({ phase: 'idle', result: null, errorCode: '' });
  ...
  async function validateDraft() {
    if (state.phase !== 'succeeded' || !state.result?.operationId || !state.result?.externalRecordId || auth.phase !== 'authenticated' || validation.phase === 'validating') return;
    setValidation({ phase: 'validating', result: null, errorCode: '' });
    try {
      const response = await fetch('/api/agent/xchange/actions/validate', {
        method: 'POST', credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'X-NexAeon-CSRF': auth.csrfToken },
        body: JSON.stringify({ executeOperationId: state.result.operationId, agentId: 'xchange', actionType: 'validate', contractVersion: 'v1', schemaVersion: 'v1' }),
      });
      setValidation({ phase: 'completed', result: await parse(response), errorCode: '' });
    } catch (error) {
      setValidation({ phase: 'failed', result: null, errorCode: error.code || 'NOTION_VALIDATION_READ_FAILED' });
    }
  }

  return (
    <section className="xchange-draft-preview-panel" data-testid="xchange-draft-preview-panel" data-phase={state.phase}>
      <div className="xchange-preview-heading"><div><span className="content-tag">Stage 5-3E-F · Draft delivery validation</span><h2>{copy.title}</h2><p>{copy.intro}</p></div></div>
      ...
      {state.preview ? (
        <div className="agent-action-preview xchange-structured-preview" data-testid="xchange-structured-preview">
          ...
          <dl>
            ...
            <div><dt>{copy.generation}</dt><dd>{state.preview.modelGeneration?.provider || 'mock'} · {state.preview.modelGeneration?.model || 'deterministic-v1'} · {state.preview.modelGeneration?.generationMode || 'deterministic'}{state.preview.modelGeneration?.fallbackUsed ? ` · ${copy.fallback}` : ''}</dd></div>
          ...
          {state.result ? <div className="xchange-execution-success" data-testid="xchange-execution-success"><strong>{copy.created}</strong><dl><div><dt>{copy.operation}</dt><dd>{state.result.operationId}</dd></div><div><dt>{copy.status}</dt><dd>Succeeded · Draft · Private · Published=false</dd></div><div><dt>{copy.writes}</dt><dd>1 · performed 1</dd></div><div><dt>{copy.createdAt}</dt><dd>{state.result.createdAt}</dd></div><div><dt>{copy.recordId}</dt><dd>{state.result.externalRecordId}</dd></div></dl><p>{copy.notPublished}</p><p>{copy.readOnlyValidation}</p><button className="mvp-action-button" type="button" onClick={validateDraft} disabled={auth.phase !== 'authenticated' || validation.phase === 'validating'}>{validation.phase === 'validating' ? copy.validatingDraft : copy.validateDraft}</button>{validation.errorCode ? <p className="agent-state-message" data-state="failed" data-testid="xchange-validation-failure">{copy.validationFailed}: {errorText(lang, validation.errorCode)} ({validation.errorCode})</p> : null}{validation.result ? <section className="xchange-content-quality" data-testid="xchange-delivery-readiness"><h3>{copy.validationTitle}: {validation.result.readinessStatus}</h3><dl><div><dt>{copy.propertiesStatus}</dt><dd>{validation.result.propertiesStatus}</dd></div><div><dt>{copy.contentStatus}</dt><dd>{validation.result.contentStatus}</dd></div><div><dt>{copy.revisionStatus}</dt><dd>{validation.result.revisionStatus}</dd></div><div><dt>{copy.durationStatus}</dt><dd>{validation.result.durationStatus}</dd></div><div><dt>{copy.safetyValidation}</dt><dd>{validation.result.safetyStatus}</dd></div><div><dt>{copy.blockCounts}</dt><dd>{validation.result.actualTopLevelBlocks} / {validation.result.expectedTopLevelBlocks} top-level · {validation.result.actualTotalBlocks} / {validation.result.expectedTotalBlocks} total</dd></div><div><dt>{copy.validationWrites}</dt><dd>{validation.result.writesPerformed}</dd></div></dl><strong>{copy.missingSections}</strong><ContentValue value={validation.result.missingSections || []} /><strong>{copy.mismatches}</strong><ContentValue value={[...(validation.result.mismatchedProperties || []), ...(validation.result.mismatchedSections || [])]} /><strong>{copy.validationWarnings}</strong><ContentValue value={validation.result.warnings || []} /></section> : null}</div> : null}
        </div>
      ) : null}
    </section>
  );
