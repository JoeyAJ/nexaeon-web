...
const OPERATION_ERROR_STATUS = Object.freeze({
  ...
  SCHEMA_MISMATCH: 503, NOTION_CONFIGURATION_MISSING: 503, NOTION_REQUEST_FAILED: 502, NOTION_INVALID_RESPONSE: 502, NOTION_BODY_APPEND_FAILED: 502,
  ...
  AUDIT_SELECT_FIELD_UNKNOWN: 503, AUDIT_SELECT_VALUE_INVALID: 503,
...
async function handleXchangeOperationRequest(req, res) {
  privateJson(res);
  if (req.method !== 'POST') {
    res.setHeader('Allow', 'POST');
  ...
  try {
    ...
    return res.status(OPERATION_ERROR_STATUS[errorCode] || 500).json({
      ok: false,
      errorCode,
      ...(Array.isArray(error?.rejectedFields) ? { rejectedFields: error.rejectedFields } : {}),
      writesPerformed: error?.writesPerformed || 0,
      ...(error?.partialExternalWrite ? {
        notionPageCreated: true, pageCreated: true, bodyComplete: false,
        bodyBlocksWritten: error.bodyBlocksWritten || 0, bodyAppendBatches: error.bodyAppendBatches || 0,
        partialExternalWrite: true, externalRecordId: error.externalRecordId,
      } : {}),
    });
  }
}
...
export default async function handler(req, res) {
  ...
  if (req.query?.agent === 'xchange') {
    if (req.query?.operation) {
      await handleXchangeOperationRequest(req, res);
      return;
    }
